...
class ResourceControllerTest extends UnitTestCase {
	...
	// Fork semantics: with apiResourceWriteWorkspaceId = -1 (unset), an existing
	// backend user's workspace is PRESERVED for workspace-aware writes — upstream
	// forces live (0) here instead.
	public function testCreateActionPreservesExistingBackendUserWorkspace(): void {
		...
		$this->assertSame(200, $response->getStatusCode());
		$this->assertSame(99, $backendUser->workspace);
		$this->assertSame(['uid' => 99], $backendUser->workspaceRec);
		$this->assertSame(99, $backendUser->user['workspace_id']);
	}
}
